...
icon = PIL.Image.open("guvilogo.png")
st.set_page_config(page_title= "BizCardX: Extracting Business Card Data with OCR",
                     layout= "wide",
                     initial_sidebar_state= "expanded",
                     menu_items={'About': """This OCR App is created by Venkatesh Kumar S, GUVI DataScience, Batch-D6162."""})
...
with st.sidebar:
    if st.button('About'):
        st.write('---')
        st.markdown('This project extracts the text data from the uploaded business card image using EasyOCR. The extracted data is then stored in MySQL database and CRUD operations are performed.')
 
# Initialising EasyOCR
reader=easyocr.Reader(['en'],gpu=False)

# Connecting MySQL Database
mydb=mysql.connector.connect(host='localhost',user='root',password='ymax1126',database='ocr')
...
if option == 'Read':
    ...
    mycursor.execute('select Image,Company_Name,Name,Designation,Phone_Number,Email,Website,Address,PinCode from BizCard')
    df_read = pd.DataFrame(mycursor.fetchall(),columns=['Image','Company_Name','Name','Designation','Phone_Number','Email','Website','Address','PinCode'])
    st.write(df_read)
...
# Modifying Extracted Data
    ...
    mycursor.execute('select Company_Name,Name,Designation,Phone_Number,Email,Website,Address,PinCode from BizCard where Name=%s',(selected_card,))
    ...
    if st.button('Commit Changes'):
        mycursor.execute("""UPDATE BizCard SET Company_Name=%s,Name=%s,Designation=%s,Phone_Number=%s,Email=%s,Website=%s,Address=%s,PinCode=%s
                                WHERE Name=%s""", (Company_Name,Name,Designation,Phone_Number,Email,Website,Address,PinCode,selected_card))
        mydb.commit()
